...
const formatSafeDate = (dateStr: string | null | undefined, format: 'locale' | 'localeString' = 'locale'): string => {
  try {
    ...
    return format === 'localeString' ? date.toLocaleString() : date.toLocaleDateString();
  } catch (error) {
...
export const ResultsPage: React.FC = () => {
    ...
    try {
      ...
      if (testResult) {
        ...
        testInfo = {
          ...
          date: formatSafeDate(testResult.start_time),
  ...
  const handleDownloadPdf = () => {
    ...
    try {
      ...
      doc.text(`Date: ${formatSafeDate(selectedTest.start_time, 'localeString')}`, 16, 107);
          ...
          <TableBody>
            {results.map((result) => (
              <TableRow key={result.attempt_id}>
                ...
                <TableCell>
                  {formatSafeDate(result.start_time)}
                </TableCell>                <TableCell>
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <Box sx={{ pl: 1 }}>
                      <Typography sx={{ mb: 1, display: 'flex' }}>
                        ...
                        <Box component="span" sx={{ ml: 1 }}>{formatSafeDate(selectedTestResult?.start_time || selectedTest.start_time, 'localeString')}</Box>
                    ...
                    return (
                      <Box sx={{ position: 'relative', zIndex: 1 }}>                        {/* Score circle */}
                        <Box sx={{ 
                          ...
                          <Box sx={(theme) => {
                            // Get theme-safe colors
                            const getScoreColor = (scoreClass: string) => {
                              try {
                                ...
                                return colorMap[scoreClass] || theme.palette.primary;
                              } catch (error) {
                                console.warn('Color theme error, using primary:', error);
                                return theme.palette.primary;
                              }
                            };
                            
                            const colorPalette = getScoreColor(scoreClass);
                            ...
                            return {
                              ...
                              justifyContent: 'center',
                              border: '10px solid',
                              borderColor: colorPalette.light || colorPalette.main,
                              ...
                              '&::before': {
                                ...
                                left: 0,
                                width: '100%',
                                height: `${Math.min(Math.max(correctPercent, 0), 100)}%`,
                                backgroundColor: colorPalette.light || colorPalette.main,
